...
bool SetConfigPath(const std::wstring& configPath)
{
    ...
    result = RegSetValueExW(hKey, CONFIG_VALUE_NAME, 0, REG_SZ, reinterpret_cast<const BYTE*>(configPath.c_str()),
...
int __cdecl wmain(int argc, wchar_t* argv[])
{
    ...
    HRESULT hr = SwDeviceCreate(L"IddSampleDriver", L"HTREE\\ROOT\\0", &createInfo, 0, nullptr, CreationCallback,
                                &hEvent, &hSwDevice);
    if (FAILED(hr))
    {
