...
void PrintUsage()
{
    ...
    std::wcout << L"Options:\n";
    std::wcout << L"  --config <path>        Load monitor configuration from JSON file\n";
    std::wcout << L"  --template <name>      Load predefined template (e.g., dual-1080p, 2x2-grid)\n";
...
bool SetConfigPath(const std::wstring& configPath)
{
   ...
   REG_OPTION_NON_VOLATILE,
    ...
    result = RegSetValueExW(
        ...
        0,
        REG_SZ,
        reinterpret_cast<const BYTE*>(configPath.c_str()),
static_cast<DWORD>((configPath.length() + 1) * sizeof(wchar_t))
    );
...
bool ValidateConfigFile(const std::wstring& configPath)
{
 ...
 std::ifstream file(configPath);
  ...
  content.find("\"height\"") == std::string::npos)
    {
   std::wcerr << L"Error: Configuration missing required fields (id, width, height)\n";
  return false;
    }
